...
def floyd(graph, num_vertices):
    ...
    path = [[None] * num_vertices for _ in range(num_vertices)]  # Ma trận đường đi
    ...
    for i in range(num_vertices):
        for j in range(num_vertices):
            if i == j:
                ...
                dist[i][j] = graph[i][j]
                path[i][j] = i  # Đường đi từ i đến j ban đầu qua chính i
            ...
            for j in range(num_vertices):
                ...
                if dist[i][k] != float('inf') and dist[k][j] != float('inf'):
                    if dist[i][j] > dist[i][k] + dist[k][j]:
                        ...
                        path[i][j] = path[k][j]  # Cập nhật đỉnh trung gian
...
# Hàm truy vết và in đường đi ngắn nhất
def print_path(path, start, end):
    ...
    route.reverse()  # Đảo ngược để có đường đi từ start -> end
...
def main():
    ...
    result, path, has_negative_cycle = floyd(graph, num_vertices)
    ...
    print("\nMa trận đường đi (None biểu thị không có đường đi):")
    for row in path:
        print(row)

    # Hiển thị đường đi giữa các cặp đỉnh
    print("\nCác đường đi ngắn nhất giữa các đỉnh:")
    for i in range(num_vertices):
        for j in range(num_vertices):
            if i != j:
                print(f"Đường đi từ {i} đến {j}: {print_path(path, i, j)}")
